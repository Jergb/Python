...
def filtro(filtrar,variable,i,q,li,ls):
    ...
    try:
        ...
        if len(val)>0:
            if filtrar[variable][i]>np.mean(val):
                filtrar[variable][i] = np.min(val)
            else:
                filtrar[variable][i] = np.max(val)
...
def filtrar_variables(variable,li,ls):
    'Ajusta los valores atípicos en las mediciones de la variable v'
    global filtrado
    
    for i in range(len(filtrar[variable])):
        if filtrar[variable][i] >= ls:
            filtro(filtrado,variable,i,filtrado[variable].quantile(.75),li,ls)
        
        elif filtrar[variable][i] <= li:
            filtro(filtrado,variable,i,filtrado[variable].quantile(.25),li,ls)
    
    filtrado[variable].apply(lambda x: max(x, filtrado[variable].quantile(.01)))
...
def cambio(variable,ventana):
    ...
    for i in rolling_std.index:
        if rolling_std.loc[rolling_std]>2.5:
            try:
                filtrado[variable].loc[indice] = rep
            except TypeError:
                pass
    return
