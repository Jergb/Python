...
def separa_datos(datos, edge):
    ...
    registros = [len(separados[x]) for x in range(len(separados))]
...
def info_periodo(dataset,inicio,fin):
    ...
    faltantes = len(df) - df.count()
    r_registros = (registros / len(df))*100
    n_registros = (faltantes / len(df))*100
    ...
    with plt.style.context(('seaborn')):
        ...
        for v in [v1,v2]:
            ...
            try:
                ...
                df.loc[pd.DataFrame(ind).iloc[:,0],'o'] = pd.DataFrame(ind).iloc[:,1].values
               
                marca = v
                pos = df.columns[-1]
                if marca == v1:
                    plt.scatter(df[pos],df[v2],color='r')
                    plt.subplot(337)
                    plt.plot(df[pos],'ro')
                    plt.subplot(338)
                    plt.hist(df[pos][~np.isnan(df[pos])],
                             facecolor='r')
                    ind = []
                                        
                if marca == v2:
                    plt.scatter(df[v1],df[pos],color='r')   
                    plt.subplot(339)
                    plt.plot(df[pos],'ro')
                    plt.subplot(334)
                    plt.hist(df[pos][~np.isnan(df[pos])],
...
def filtro(variable,i,j,q,li,ls):
    'Iguala el valor de una medición a un determinado valor límite'
    global filtrado
    try:
        
        d=len(filtrado[variable].iloc[0:i])/(60*24)
        val=[filtrado[variable][i-(60*24)*(1+j)] for j in range(int(d)) if d>1]
        val+=[filtrado[variable][j]]
        val = [i for i in val if i>li and i<ls]

        if len(val)>0:
            filtrado[variable][i] = np.mean(val)                      

    except KeyError:
        filtrado[variable][i] = 0
        
    return


def filtrar_saltos(variable,li,ls,start,j):
    'Ajusta los valores atípicos en las mediciones de la variable v'
    global filtrado
    
    for i in range(start,len(filtrado[variable])):
        
        if filtrado[variable][i] < li or filtrado[variable][i] > ls:
            filtro(variable,i,i-j,0,li,ls)
            
        if abs(filtrado[variable][i]-filtrado[variable][i-j]) >= filtrado[variable].std():
            filtro(variable,i,i-j,0,li,ls)
            
    #for i in range(len(filtrado[variable]),start,-1):
     #   if abs(filtrado[variable][i]-filtrado[variable][i-j]) >= filtrado[variable].std():
      #      filtro(variable,i,i-j,0,li,ls) 
    
    filtrado[variable].apply(lambda x: max(x, filtrado[variable].quantile(.99)))
    filtrado[variable].apply(lambda x: min(x, filtrado[variable].quantile(.01)))
...
def ajuste_variacion(variable,ventana):
    ...
    nvalores=suave[rolling_std>2.5].index
    nnvalores=[suave.index.get_loc(x) for x in nvalores]
    #print('nvalores: ',nvalores)
    rolling_median=suave.rolling(ventana).mean()
    
    # Hace una ventana deslizante desde la última fila
    rolling_median_inv=suave.iloc[::-1].rolling(ventana).mean().iloc[::-1]
    ...
    # redondea los valores de rolling_median
